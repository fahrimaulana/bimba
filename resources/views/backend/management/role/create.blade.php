...
@section('content')
    <ul class="blocks-xs-100 blocks-lg-2">
        ...
        <li class="block-70 no-mrg-btm">
            <form class="form-horizontal" id="form" action="{{ route(strtolower(platform()) . '.management.role.store') }}" method="POST">
                {!! csrf_field() !!}
                                ...
                                @foreach ($permissionGroups->chunk(ceil(count($permissionGroups) / 2))[0] as $groupName => $permissions)
                                    <h4 class="example-title">
                                        ...
                                        <div class="pull-right" style="padding-right: 8px;">
                                            Select All
                                            <input type="checkbox" class="icheckbox-primary selectAll" data-group="{{ $groupName }}" data-plugin="iCheck" data-checkbox-class="icheckbox_flat-blue" checked>
                                        </div>
                                    </h4>
                                    <table class="table">
                                        ...
                                        <tr>
                                            ...
                                            <td><input type="checkbox" class="icheckbox-primary" data-group="{{ $groupName }}" value="{{ $permission->id }}" name="permission_ids[]" data-plugin="iCheck" data-checkbox-class="icheckbox_flat-blue" checked></td>
                                        </tr>
                                        @endforeach
                                    </table>
                                @endforeach
