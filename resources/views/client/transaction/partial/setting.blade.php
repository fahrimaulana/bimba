                ...
                @include('inc.error-list')
                    <div class='row'>
                        <div class='form-group col-md-6'>
                            ...
                            <select class='form-control select2' name='student_id' id="student" required>
                                ...
                                <option value='{{ $student->id }}' {{ old('student_id') == $student->id ? 'selected' : '' }} data-student-id="{{ $student->id }}" data-department-id="{{ optional($student->department)->id }}" data-department-price="{{ optional($student->department)->price }}">{{ $student->nim .' - '. $student->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class='form-group col-md-6'>
                            <label class='control-label'>Daftar</label>
                            <select class='form-control select2 registration' name="settings[0][registration]" id="select2">
                                <option></option>
                                @foreach($departments as $department)
                                <option value="{{ $department->price }}" {{ old('settings[0][registration]') == $department->id ? 'selected' : '' }} data-department-id="{{ $department->id }}" data-department-price="{{ $department->price }}">{{ thousandSeparator($department->price) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="row">
                        <div class="form-group col-md-3">
                            ...
                            <div class="input-group">
                                <input type="number" name="tuition_month" class="form-control disabled-input tuition-month" value="{{ old('tuition_month') }}">
                    ...
                    <div class='row'>
                        <div class='form-group col-md-4'>
                            ...
                            <select class='form-control select2 disabled-input tuition-voucher' name='voucher_id'>
                                <option></option>
                            </select>
                        </div>
                        <div class='form-group col-md-4'>
                            ...
                            <input type="hidden" name="settings[2][event]" class="separator-hidden" value="{{ old('settings[2][event]', isset($transactionDetailEvent) ? $transactionDetailEvent->total : '0') }}" >
                        </div>
                        <div class='form-group col-md-4'>
                            <label class='control-label'>Lain-Lain</label>
                            <input type='text' name='settings[3][other]' class='form-control separator disabled-input student-other' value="{{ thousandSeparator(old('other', isset($transactionDetailOther) ? $transactionDetailOther->total : '0')) }}" min="0" >
                            <input type="hidden" name="settings[3][other]" class="separator-hidden" value="{{ thousandSeparator(old('other', isset($transactionDetailOther) ? $transactionDetailOther->total : '0')) }}" >
                        </div>
                    </div>
            </div>
            <div class="panel-heading">
            ...
            <div class="panel-body">
                ...
                <div class="row iTemplate" id="product-template">
                    ...
                    <div class="form-group col-md-2">
                        <input type="text" class="form-control disabled-input product-qty" value="{{ old('product_qty', '1') }}" min="1" placeholder="0">
                    </div>
                    <div class="form-group col-md-2">
                ...
                <div class="row">
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control select2" name="payment_method" required="">
                            <option></option>
                            <option value="Cash" {{ old('payment_method') == 'Cach' ? 'selected' : '' }}>Cash</option>
                            <option value="Edc" {{ old('payment_method') == 'Edc' ? 'selected' : '' }}>Edc</option>
                            <option value="Transfer" {{ old('payment_method') == 'Transfer' ? 'selected' : '' }}>Transfer</option>
                        </select>
                    </div>
                    <div class="form-group col-md-4">
                        <label class="control-label">Kwitansi No</label>
                        <input type="text" name="receipt_no" class="form-control" value="{{ old('receipt_no', isset($transaction) ? $transaction->receipt_no : '') }}" required>
                    </div>
                    <div class="form-group col-md-4">
