...
@section('head')
    <title>Report Staff Profile | {{ env('APP_NAME') }}</title>
...
@push('breadcrumb')
    ...
    <li class="breadcrumb-item active">Report Staff Profile</li>
@endpush
...
@section('content')
    <div class="panel">
        <div class="panel-body">
            ...
            <form class="form-horizontal" action="{{ route('client.report.staff.profil.display-pdf') }}" method="POST">
                {!! csrf_field() !!}
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="sort_by" class="form-control select2" data-placeholder="Choose One">
                            <option></option>
                            <option value="nik" {{ (old('sort_by') == 'nik') ? 'selected' : '' }}>NIK</option>
                            <option value="staff_name" {{ (old('sort_by') == 'staff_name') ? 'selected' : '' }}>Nama</option>
                            <option value="jabatan" {{ (old('sort_by') == 'jabatan') ? 'selected' : '' }}>Jabatan</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="form-group col-sm-12">
                        <div class="row">
                            ...
                            <div class="form-group col-sm-6">
                                <button type="submit" formaction="{{ route('client.report.staff.profil.download-pdf') }}" class="btn btn-danger w100"><i class="fa fa-file-pdf-o"></i> Download Di  PDF</button>
                            </div>
                            <div class="form-group col-sm-6">
                                <button type="submit" formaction="{{ route('client.report.staff.profil.download-excel') }}" class="btn btn-success w100"><i class="fa fa-file-excel-o"></i> Download Di Excel</button>
                            </div>
                            <div class="form-group col-sm-6">
                                <button type="submit" formaction="{{ route('client.report.staff.profil.download-csv') }}" class="btn btn-info w100"><i class="fa fa-file-excel-o"></i> Download Di CSV</button>
                            </div>
                        </div>
                    </div>
                </div>
