...
@section('head')
    <title>Grade List | {{ env('APP_NAME') }}</title>
...
@section('content')
                ...
                <div class="col-xs-12">
                    <div class="m-b-15">
                        <a id="add-btn" class="btn btn-primary white">
                            <i class="icon wb-plus" aria-hidden="true"></i> Add Grade
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="panel">
        <div class="panel-body" id="add-form" {!! count($errors) == 0 ? "style='display: none;'" : '' !!}>
            ...
            <form class="form-horizontal" id="form" action="{{ route('client.master.grade.store') }}" method="POST" enctype="multipart/form-data">
                {!! csrf_field() !!}
                <div class='row'>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#datatable').dataTable({
                ...
                ajax: {
                    method: 'POST',
                    url : '{{ route('client.master.grade.data') }}',
                    headers: {
