...
@section('head')
    <title>Staff Position List | {{ env('APP_NAME') }}</title>
...
@section('content')
    <div class="panel">
        <div class="panel-body">
            ...
            @can('create-staff-position')
            <div class="row">
                <div class="col-xs-12">
                    <div class="m-b-15">
                        <a id="add-btn" class="btn btn-primary white">
                            <i class="icon wb-plus" aria-hidden="true"></i> Add Staff Position
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="panel">
        <div class="panel-body" id="add-form" {!! count($errors) == 0 ? "style='display: none;'" : '' !!}>
            <h3>Add New Staff Position</h3>
            @include('inc.error-list')
            <form class="form-horizontal" id="form" action="{{ route('client.master.staff-position.store') }}" method="POST" enctype="multipart/form-data">
                {!! csrf_field() !!}
                <div class='row'>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#datatable').dataTable({
                ...
                ajax: {
                    method: 'POST',
                    url : '{{ route('client.master.staff-position.data') }}',
                    headers: {
