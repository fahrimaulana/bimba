...
@section('head')
    <title>Public Relation Status List | {{ env('APP_NAME') }}</title>
...
@section('title')
    Public Relation Status List
@endsection

@push('breadcrumb')
    <li class="breadcrumb-item active">Public Relation Status</li>
@endpush
...
@section('content')
    <div class="panel">
        <div class="panel-body">
            ...
            @can('create-public-relation-status')
            <div class="row">
                <div class="col-xs-12">
                    <div class="m-b-15">
                        <a id="add-btn" class="btn btn-primary white">
                            <i class="icon wb-plus" aria-hidden="true"></i> Add Public Relation Status
                        </a>
                    </div>
                </div>
            </div>
    ...
    <div class="panel">
        <div class="panel-body" id="add-form" {!! count($errors) == 0 ? "style='display: none;'" : '' !!}>
            <h3>Add New Public Relation Status</h3>
            @include('inc.error-list')
            <form class="form-horizontal" id="form" action="{{ route('client.master.public-relation-status.store') }}" method="POST" enctype="multipart/form-data">
                {!! csrf_field() !!}
                <div class='row'>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $('#datatable').dataTable({
                ...
                ajax: {
                    method: 'POST',
                    url : '{{ route('client.master.public-relation-status.data') }}',
                    headers: {
